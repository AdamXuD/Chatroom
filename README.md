...
这是由综合六组全体成员努力编写的一个聊天室的练手项目，旨在提高对C/C++与网络编程的理解，本聊天室虽然只是运行在shell上的命令行程序，但还是力求提高其易用性，用户体验。让C/C++初学者，使用者摆脱命令行界面简陋，交互体验差等死板固有印象。
...
这个项目由我们综合六组的组员们鼎立配合支持才能完成。

| 功能                                                         | 将其实现的成员       |
| ------------------------------------------------------------ | -------------------- |
| 好友列表（好友的添加与删除，获取在线好友，特别关心和黑名单等操作） | 王小璐 魏向前        |
| 群聊的逻辑部分（创建群聊，加入群聊，退出群聊，踢出群聊，群聊消息在服务器上的转发，以及数据库中如何保存一个群与该群的成员） | 王一帆 向陟昕 王瑞达 |
| 群聊的权限部分（设定管理员，以及如何在数据库上保存群员和群主管理员的权限，以及如何确保只有管理员与群主能对群员做出修改） | 杨奉源 杨谦诚        |
| 消息保存与用户数据保存与读取（私聊聊天记录保存，群聊聊天记录保存） | 肖进 杨鹏涛          |
| 用户交互                                                     | 徐东浩               |

## 关于Bug以及该项目的维护

若是有什么严重的bug请直接跟团队成员们联系。
